...
let uclog = OSLog(subsystem: "ca.unitcircle.Konnex", category: "General")

// See https://www.bignerdranch.com/blog/migrating-to-unified-logging-swift-edition/
// It has a section on "... gathering a sysdiagnose."

#if true
func uc_os_log(_ text: StaticString, _ value: CVarArg) {
    os_log(text, log: uclog, type: .info, value)
    os_log(.info, log: uclog, text, value)
}

func uc_os_log(_ text: StaticString) {
  os_log(text, log: uclog, type: .info)
}

func uc_os_log(_ text: String) {
    os_log("%{pubilc}@", log: uclog, type: .info, text)
}
